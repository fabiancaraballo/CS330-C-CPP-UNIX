...
void Shrinker::Execute()
{
	...
	img.setBuffer(input1->getWidth() / 2, input1->getHeight() / 2);
		...
		for(j = 0; j < input1->getHeight() / 2; j++) {
	              img.getBuffer()[j*input1->getWidth()/2+i].r = input1->getBuffer()[4 * j * input1->getWidth()/2 + 2*i].r;
		      img.getBuffer()[j*input1->getWidth()/2+i].g = input1->getBuffer()[4 * j * input1->getWidth()/2 + 2*i].g;
	              img.getBuffer()[j*input1->getWidth()/2+i].b = input1->getBuffer()[4 * j * input1->getWidth()/2 + 2*i].b;
		}
...
void TBCombine::Execute()
{
	...
	for (i = 0; i < topWidth*topHeight; i++) {
